namespace Applications.Services.Validators
{
    public static class ServiceOrderDateValidator
    {
        ...
        public static void ValidateTryInDate ( List<ProductionStage> stages, DateTime tryInDate )
        {
            ...
            if ( stages.All ( s => s.DateOut != null ) )
                throw new UnprocessableEntityException ( "Cannot send to try-in if there is no open stage." );

            var lastStage = stages.OrderByDescending(s => s.DateIn).First();
